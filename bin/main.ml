...
let setInputFile name =
  ...
  then
    ...
    failwith "Error: More than one input file specified."
...
let checkInput () =
  ...
  then
    ...
    (if Sys.file_exists !inputFile
     then
       (lexbuf := Lexing.from_channel (open_in !inputFile);
        !lexbuf.Lexing.lex_curr_p <- {
          !lexbuf.Lexing.lex_curr_p with
          Lexing.pos_fname = !inputFile } )
     else
       failwith ("Error: Invalid input file: `" ^ !inputFile ^ "'.") ) 
             
let usageMsg =
  "Usage: main [options]\n" ^ "options are: "

let specList = 
  [("-i", Arg.String setInputFile,
    " Specifies a file from which to read input.");
   ("--input", Arg.String setInputFile,
    " Specifies a file from which to read input.");
...
let process_proof () =
  ...
  | Uterms.Apply(Uterms.Remove(name),args,withs) ->
    ...
    Prover.apply name (List.map (fun x -> match x with | Uterms.Keep(arg) | Uterms.Remove(arg) -> arg) args) withs
  | Uterms.Case(Uterms.Keep(hyp)) ->
    Prover.case false hyp
  | Uterms.Case(Uterms.Remove(hyp))->
    Prover.case true hyp
  | Uterms.Exists(utm) ->
        (* weak type checking/inference must be done on utm using current proof state
           then call Prover.exsits*)
    let nvars =
      List.filter (fun (_,t) -> Term.is_var Term.Nominal t) (Prover.get_sequent ()).Sequent.vars
    in
    let evars =
    ...
    (match (Prover.get_sequent ()).goal with
     | Formula.Exists((_,ty)::_, _) ->
      ...
      Prover.exists term
     | _ ->
        prerr_endline "Goal formula not existential") 
  | Uterms.Split ->
    Prover.split ()
  | Uterms.Left ->
    Prover.left ()
  ...
  | Uterms.Weaken(clear,utm) ->
     ...
     (match clear with
      | Uterms.Keep(name) ->
         Prover.weaken false name t
      | Uterms.Remove(name) ->
        Prover.weaken true name t)
  | Uterms.PermuteCtx(clear, uctx) ->
    ...
    (match clear with
    | Uterms.Keep(name) ->
      Prover.permute_ctx false name g
    | Uterms.Remove(name) ->
      Prover.permute_ctx true name g)
  | Uterms.Strengthen(clear) ->
     (match clear with
      | Uterms.Keep(name) ->
         Prover.strengthen false name
      | Uterms.Remove(name) ->
         Prover.strengthen true name)
  | Uterms.Inst(clear, uwiths) ->
    (match clear with
    | Uterms.Keep(name) ->
      Prover.inst false name uwiths
    | Uterms.Remove(name) ->
       Prover.inst true name uwiths)
  | Uterms.Prune(id) ->
     Prover.prune id
  | Uterms.Unfold(hypnameop, withs) ->
     Prover.unfold hypnameop withs
  | Uterms.AppDfn(defname, hypnameop, withs) ->
     Prover.applydfn defname hypnameop withs
