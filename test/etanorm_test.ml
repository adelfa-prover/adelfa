...
let eta_normalize t = eta_normalize t
...
let tests =
  ...
  >::: [ ("x\\ n x = n"
         ...
         assert_term_equal n (eta_normalize ([ "x", ity ] // (n ^^ [ x ]))))
       ; ("x\\ y\\ n x y = n"
         >:: fun () ->
         let n = nominal_var "n" iiity in
         let x = const ~ts:2 "x" ity in
         let y = const ~ts:2 "y" ity in
         assert_term_equal n (eta_normalize ([ "x", ity; "y", ity ] // (n ^^ [ x; y ]))))
       ; ("x\\ y\\ n y x = x\\ y\\ n y x"
         >:: fun () ->
         let n = nominal_var "n" iiity in
         let x = const ~ts:2 "x" ity in
       ...
       ; ("x\\ n c x = n c"
         ...
         assert_term_equal (n ^^ [ c ]) (eta_normalize ([ "x", ity ] // (n ^^ [ c; x ])))
         )
       ; ("x\\ n (y\\ c y) x = nc"
         >:: fun () ->
