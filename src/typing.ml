...
(** [wf_type] sign ctx ty checks if a type is well-formed

    @param sign The LF signature
    @param ctx The LF context
    @param ty The type to check for well-formedness

    @raise NotLFType if the type is not well-formed *)
let wf_type sign _ ty =
  match ty with
  | Var v when v.tag = Constant ->
    if Signature.is_type sign v.name then () else raise (NotLFType ty)
  | Type -> ()
  | Pi (_, _) -> () (* TODO finish *)
  | _ -> ()
;;
...
let rec get_type sign (ctx : Term.lftyctx) tm =
  ...
  | Var v when v.tag = Constant ->
    ...
    then (
      ...
      od.Signature.typ)
    else raise (NotLFTerm tm)
  | Var v when v.tag = Nominal ->
    if List.mem_assoc v ctx then List.assoc v ctx else raise (NotLFTerm tm)
  | App (h, _) ->
...
and check_type sign (ctx : Term.lftyctx) tm ty =
  ...
  | Var _ | App _ ->
    ...
    if eq found_ty ty then ty else raise (TypeMismatch (ty, found_ty))
    ...
    let is_instance (Context.B (vars, entries)) =
      ...
      then false
        ...
        let var_subst =
          List.map
            (fun (id, ty) ->
              id, Term.app (Term.fresh (Type.tyarrow ntys ty)) (List.map snd nvars))
            vars
        in
        let nominal_subst =
...
let rec match_tys (l1 : Type.ty list) (l2 : Type.ty list) =
  match l1, l2 with
  | h1 :: l1', h2 :: l2' ->
    if Type.eq h1 h2 then match_tys l1' l2' else raise (TypeError (h1, h2))
  | _ -> ()
;;
...
let apply_ty fty argtys =
  ...
  | Type.Ty (atys, base), _ ->
    ...
    else raise (TypeError (Type.tyarrow atys Type.oty, Type.tyarrow argtys Type.oty))
;;
...
let rec infer_ty env tm =
  ...
  | Lam (vs, body) ->
    ...
    Type.tyarrow typs (infer_ty (List.rev typs @ env) body)
  | App (h, args) ->
...
let of_type tm ty =
  ...
  if Type.eq got_type ty then true else raise @@ TypeError (ty, got_type)
;;
