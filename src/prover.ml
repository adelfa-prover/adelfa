...
let schemas:(string * Context.ctx_schema) list ref =
  ref []
let add_schema id s =
  schemas := (id,s) :: !schemas
let clear_schemas () = schemas := []
(* Raises Not_found if schema of given name is not in the schema list *)
let lookup_schema id =
  List.assoc id !schemas

(* 3. The available lemmas *)
let lemmas:(string * Formula.formula) list ref =
  ref [] 
let add_lemma id f =
  lemmas := (id, f) :: !lemmas
let clear_lemmas () = lemmas := []
let lookup_lemma id =
  List.assoc id !lemmas
  ...
  let rec annotate i gs =
    ...
    | g::rest ->
      (fun () ->
        ...
        sequent.next_subgoal_id <- 1) :: annotate (i+1) rest
  in
...
let add_definition dfn =
  dfns := dfn :: !dfns
let clear_definitions () = dfns := []
let lookup_definition id =
  snd (List.assoc id !dfns)
let get_propty_lst () =
  List.map (fun (x,(y,_)) -> (x,y)) !dfns

             
let undo_stack = ref []

let state_snapshot () =
  (copy_sequent (), !subgoals, Term.get_bind_state (), Term.get_varcount (), Context.get_varcount ())
...
let display_state () =
  ...
  List.iter
    (fun subgoal ->
       subgoal ();
       Format.printf "@[<1>Subgoal %s%sis:@\n%a@]@\n@\n%!"
         sequent.name
         (if sequent.name = "" then "" else " ")
         Print.pr_formula sequent.goal)
    !subgoals ;
 ...
 try
  ...
  | Tactics.NotLlambda ->
    (prerr_endline "cases failed: not llambda";
     undo ())
  | Not_found ->
    prerr_endline ("No assumption of name `"^hyp^"'."); undo ()
    
let exists tm =
  save_undo_state ();
  try
    Tactics.exists sequent tm
  with
  | Typing.TypeError(exp,got) ->
    (prerr_endline ("Type Error: Expected type\n"^
                       (Print.string_of_ty exp)^
                       "\nbut found type\n"^
                       (Print.string_of_ty got));
     undo ())
  | Tactics.InvalidTerm _ ->
     (prerr_endline "Bad term"; undo ())
...
let search () =
  ...
  | Formula.Atm _ | Formula.Prop _ ->
    ...
    with Tactics.Success -> next_subgoal ())
  | Formula.Top ->
     next_subgoal ()
  | _ ->
    ...
    (fun (id, cexpr) ->
      try
        ...
        List.iter
          (Sequent.add_var sequent)
          (List.map (fun (id, r) -> (id,(Option.get !r))) new_nvarctx);
        if Typing.of_schema nvars (Sequent.get_cvar_tys sequent.ctxvars) ctxtm (schemaid, lookup_schema schemaid)
        then
          (id, ctxtm)
        else
          raise (ApplyFailure("Context expression "^
                               (Print.string_of_ctxexpr ctxtm)^
                               " does not match schema "^
                               schemaid^"."))
      with
      | Not_found ->
        raise (ApplyFailure("Unknown context variable "^ id)))
    cwiths)

let freshen_formula_names f (vwiths, cwiths) =
  let support = Formula.formula_support_sans (Sequent.get_cvar_tys sequent.Sequent.ctxvars) f in
  let withs_used =
    ...
    (List.flatten
       (List.map
          (fun (_,ctxexpr) ->
               Formula.context_support_sans (Sequent.get_cvar_tys sequent.Sequent.ctxvars) ctxexpr)
          cwiths)) 
  in 
...
let term_vars_alist tag terms =
  List.map Term.term_to_pair (Term.find_var_refs tag terms)
...
let formula_free_logic_vars ctxvars formula =
  ...
  let rec get_atomic bndrs formula =
    ...
    | Formula.Ctx(cbndrs,body) ->
       let _ = bound_ctxvars := ((List.map (fun (vid, vty) -> (Context.ctx_var vid, Context.CtxTy(vty,[]))) cbndrs) @ (!bound_ctxvars)) in
  ...
  let logic_vars = List.flatten(List.map (formula_vars_alist Term.Logic ((!bound_ctxvars)@ctxvars)) atoms) in
...
let ensure_no_logic_variable ctxvarctx f =
  ...
  if logic_vars <> [] then
    raise (ApplyFailure("Found logic varaible at toplevel."))

(* ensure that no dummy context variables remain in the formula *)
let ensure_no_uninst_ctxvariable ctxvarctx f =
  let ctx_vars =
    List.filter (fun ctxvar -> not (List.mem ctxvar (List.map fst ctxvarctx)))
                (Formula.get_formula_used_ctxvars f)
  in
  if ctx_vars <> [] then
    raise (ApplyFailure("Unable to find instantiation for context variable: "^(List.hd ctx_vars)^".\nIn formula: "^(Print.string_of_formula f)))
...
let find_by_name name =
  ...
  | None ->
    try
      (Sequent.get_hyp sequent name).Sequent.formula
    with
    | Not_found -> raise (ApplyFailure("Formula `"^name^"' not found in lemmas or assumption."))
...
let apply_form f forms uws =
  ...
  let res_f = Tactics.apply_with !schemas sequent f forms withs in
  let () = ensure_no_uninst_ctxvariable (Sequent.get_cvar_tys sequent.Sequent.ctxvars) res_f in
  let () = ensure_no_logic_variable (Sequent.get_cvar_tys sequent.Sequent.ctxvars) res_f in
  res_f
...
let weaken remove id ty =
  ...
  | Tactics.Success ->
    ...
    Sequent.add_hyp sequent (Formula.atm ~ann:ann (Context.Ctx(g,(Term.term_to_var nvar,ty))) a m)
  | Tactics.InvalidName (id) ->
     prerr_endline ("`"^id^"' is not a valid type constant."); undo ()
  | Tactics.InvalidTerm (t) ->
     prerr_endline ("Given expression is not a type."); undo (); raise (Tactics.InvalidTerm(t))
...
let permute_ctx remove hyp_name ctx_expr =
  ...
  try
    ...
    if remove
    then
      Sequent.replace_hyp sequent hyp_name form'
    else
      Sequent.add_hyp sequent form'
  with
  | Not_found ->
    prerr_endline ("No hyp of name `"^hyp_name^"' found."); undo ()
    ...
    | (Formula.Atm(Context.Ctx(g,e),m,a,ann)) as f ->
       ...
       then
         (Sequent.add_hyp sequent (Option.get f_op);
          if remove
          then Sequent.remove_hyp sequent name)
       else
         undo ()
    | Formula.Atm _ ->
  ...
  let check_withs = function
    ...
    | Uterms.Vws (id, utm) ->
      ...
      then
        ...
        raise (InstError ("`"^id^"' is not an instantiable name."))
  in 
...
let inst remove name withs =
  save_undo_state ();
  try
    let _ = if (List.length withs > 1) || (List.length withs <= 0)
            then
              raise (InstError ("Exactly one instantiation must be provided."))
    in
    let form' = inst_aux name withs in
    if remove
    then
      Sequent.remove_hyp sequent name;
    Sequent.add_hyp sequent form'
  with
  | InstError str ->
...
let unfold hypop uwiths =
  try
    ...
    if Option.is_some hypop
    then
      Sequent.replace_hyp sequent (Option.get hypop) f'
    else
      sequent.goal <- f'
  with
  | ApplyFailure(str) ->
...
let applydfn defname hypnameop uwiths = 
  try
    let f =
      ...
      then
        find_by_name (Option.get hypnameop)
      else
        sequent.goal
    in
    ...
    if Option.is_some hypnameop
    then
      Sequent.replace_hyp sequent (Option.get hypnameop) f'
    else
      sequent.goal <- f'
  with
  | ApplyFailure(str) ->
