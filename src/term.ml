        ...
        let t = hnorm t in
          begin match observe t with
            ...
            | Pi(lfidtys,t) ->
                ...
                pi
                  (List.map
                     (fun (v,ty) -> (v,replace_term_vars alist ty))
                     (List.drop n lfidtys))
          ...
          begin match observe t with
            ...
            | Pi(lfidtys,t) ->
                ...
                let lfidtys' =
                  List.mapi
                    (fun i (v,tm) ->
                       (v, (hnorm (susp tm (ol+i) (nl+i) (add_dummies e i nl)))) )
                    lfidtys
                in
  ...
  let rec aux t =
    match observe (hnorm t) with
      ...
      | Lam(i, t) ->
           lambda i (aux t)
      | Pi(is,t) ->
           pi (List.map (fun (v,ty) -> (v,aux ty)) is) (aux t)
...
let rec eta_normalize t =
  ...
  | Lam(idtys, (App(h,ts))) ->
    ...
    if List.fold_left2 (fun acc bndr arg ->
                          match norm arg with
      ...
      (List.take_last n ts')
    then
      app h (List.drop_last n ts')
    else
      lambda idtys (app h ts')
...
let nominal_var name ty =
  Ptr (ref (V {name=name; tag=Nominal; ts=max_int ; ty=ty}))

let var tag name ts ty =
  if tag = Nominal then
    nominal_var name ty
  else
    Ptr (ref (V { name=name ; tag=tag ; ts=ts;  ty=ty }))
...
let rec tc (tyctx:tyctx) t =
  match observe (hnorm t) with
    ...
    | Lam(idtys,t) ->
        tyarrow (get_ctx_tys idtys) (tc (List.rev_append idtys tyctx) t)
    | _ -> assert false
...
let convert_to_nominals bndrs used =
  let rec aux bndrs used =
    ...
    | (h,_)::tl ->
       let (v,newused) = 
         fresh_wrt ~ts:3 Nominal "n" h.ty used 
       in 
       v :: (aux tl newused) 
  in 
  aux bndrs used 
