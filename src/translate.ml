...
let rec trans_term lf_sig evar_ctx logicvar_ctx nvar_ctx bvar_ctx ty_opt tm =
  let rec trans_match nvar_ctx bvar_ctx ty = function
    ...
    | UConst(pos,id) when List.mem_assoc id bvar_ctx ->
       ...
       then
         ...
         then
           (Option.get !got, nvar_ctx)
         else
           raise (TypingError(typemismatch pos ty got_ty))
       else
    ...
    | UConst(pos,id) ->
       ...
       then
         ...
         if Type.eq ty got
         then
           (Term.const id ty, nvar_ctx)
         else
           raise (TypingError(typemismatch pos ty got))
       else
         if Signature.is_type lf_sig id
         then
    ...
    | ULam(pos, (idpos,id,tyopt), tm) ->
       (match ty with
        | Type.Ty((argty::args),bty) ->
           ...
           then
             raise (TypingError(typemismatch pos (Option.get tyopt) argty)) ;
           let (tm',nvar_ctx') =
             trans_match
               nvar_ctx
    ...
    | UPi(pos, (idpos,id), typ, tm) ->
       (match ty with
        | Type.Ty([], "o") ->
           ...
           let (tm', nvar_ctx'') = trans_match nvar_ctx' ((id,ref(Some(Term.const ~ts:2 id aty)))::bvar_ctx) Type.oty tm in
  ...
  and trans_get nvar_ctx bvar_ctx = function
    ...
    | UPi(pos, (idpos,id), typ, tm) ->
       ...
       (Term.pi [(Term.term_to_var (Term.const ~ts:2 id atyp), typ')] tm', Type.oty, nvar_ctx'')
    | UApp(pos, tm1, tm2) ->
  ...
  match ty_opt with
  | Some(ty) ->
     trans_match nvar_ctx bvar_ctx ty tm
  | None ->
...
let trans_formula lf_sig schemas dfns evar_ctx logicvar_ctx ctxvar_ctx nvar_ctx formula =
  let rec trans logicvar_ctx (ctxvar_ctx : Sequent.cvar_entry list) nvar_ctx = function
    | UTop ->
       (Formula.Top, nvar_ctx)
    | UBottom ->
       (Formula.Bottom, nvar_ctx)
    | UImp(f1,f2) ->
    ...
    | UAll(locids,f) ->
       ...
       let new_logicvar_ctx =
         List.map
           (fun (x,y) ->
              (x,if Option.is_some y then ref (Some((Term.var Term.Logic x 1 (Option.get y)))) else ref None))
           idtys
       in
       let (f', nvar_ctx') =
         trans
           (List.append (List.rev new_logicvar_ctx) logicvar_ctx)
           ctxvar_ctx
           nvar_ctx
           f
       in
       ...
       then
         let tyctx = List.map (fun (x,y) -> (x,Term.get_var_ty (Option.get (!y)))) new_logicvar_ctx in
    ...
    | UCtx(loccids,f) ->
       ...
       let (f', nvar_ctx') = trans logicvar_ctx (List.append (List.rev ctxids) ctxvar_ctx) nvar_ctx f in
    ...
    | UAtm(uctx,utm,uty,ann) ->
       ...
       let nvar_ctx'' =
         List.append
           ...
           nvar_ctx'
       in
       let (a, nvar_ctx''') = trans_term lf_sig evar_ctx logicvar_ctx nvar_ctx'' [] (Some(Type.oty)) uty in
       let arity_typ =
         try
           Term.erase a
         with
    ...
    | UProp(utm) ->
       ...
       let (args', nvar_ctx') =
         try
           List.fold_right2
               (fun tm ty (args, nvars) ->
                  let (tm, nvars') = trans_term lf_sig evar_ctx logicvar_ctx nvars [] (Some ty) tm in
                  ((tm::args),nvars'))
               uargs
               argtys
               ([],nvar_ctx)
         ...
         | Invalid_argument _ ->
            ...
            then
              raise (TypingError (typemismatch (Uterms.get_pos utm) (Type.propty) (Type.tyarrow (List.take_last (num_tys - num_uargs) argtys) Type.propty)))
            else (* num_tys < num_uargs *)
    ...
    | Uterms.UProp(utm) ->
       ...
       let new_var_lst =
         (Uterms.extract_unbound_uform [] ufleft) @ (Uterms.extract_unbound_uform [] ufright) |>
           List.unique |>
           List.remove_all (fun id -> List.mem_assoc id dfns' || Signature.is_obj lf_sig id || Signature.is_type lf_sig id)
       in
       let new_nominal_ctx, new_logicvar_ctx =
         ...
         List.partition (fun id -> Str.string_match regexp id 0) new_var_lst |>
           (fun (l1, l2) -> (List.map (fun x -> (x, ref None)) l1, List.map (fun x -> (x, ref None)) l2))
       in
       let fleft = trans_formula lf_sig schemas dfns' [] new_logicvar_ctx [] new_nominal_ctx ufleft in
       let fright = trans_formula lf_sig schemas dfns' [] new_logicvar_ctx [] new_nominal_ctx ufright in
       if List.for_all (fun (_,x) -> Option.is_some !x) new_logicvar_ctx
       then if List.for_all (fun (_,x) -> Option.is_some !x) new_nominal_ctx
       then       
         let support = List.map (fun (x,y) -> (x,Term.get_var_ty (Option.get (!y)))) new_nominal_ctx in
         let tyctx = List.map (fun (x,y) -> (x,Term.get_var_ty (Option.get (!y)))) new_logicvar_ctx in
