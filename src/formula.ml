...
let rec eq f1 f2 =
  ...
  | All(vars1, f1), All(vars2, f2)
  | Exists(vars1, f1), Exists(vars2, f2) ->
    (try  
       (List.fold_left2
  ...
  | Prop(p1,args1), Prop(p2,args2) ->
     (try
         (p1 = p2)
         &&
         (List.for_all2 (fun x y -> Term.eq x y) args1 args2)
      with
      | Invalid_argument _ -> false)
  | _, _ -> false
      
(* Constructions *)
let atm ?ann:(ann=None) g m a = Atm(g,m,a,ann)
let imp l r = Imp(l,r)
...
let map_terms f t =
  let rec aux t =
    match t with
      | Atm(g,m,a,ann) -> atm ~ann:ann (context_map f g) (f m) (f a)
...
and collect_terms_ctx ctxvars = function
  ...
  | Ctx(expr,(_,t)) ->
     (t :: (collect_terms_ctx ctxvars expr))
...
let formula_support_sans ctxvars f =
  ...
  | [v] ->
    List.minus
      supp
      (context_support ctxvars (Context.Var(v)))
  | cvars ->
...
let context_support_sans ctxvars g =
  ...
  then
    List.minus
      supp
      (Context.find_var_refs
         ctxvars
         Term.Nominal
         (Context.Var(Context.get_ctx_var g)))
  else
    supp
...
let replace_formula_vars alist t =
  ...
  let rec aux alist t =
    ...
    | Atm(g,m,a,ann) ->
      atm ~ann:ann (ctx_aux alist g) (term_aux alist m) (term_aux alist a)
    | All(vs,f) ->
    ...
    | Ctx(cvars,f) ->
       ctx cvars (aux alist f)
    | Prop(p, tmlst) ->
       prop p (List.map (term_aux alist) tmlst)
  in
  aux alist t    
...
let rec replace_formula_vars_rename ~used alist t =
  ...
  let rec aux alist t =
    ...
    | All(vs,f) ->
      let alist = List.remove_assocs (List.map fst vs) alist in
      let subst_free = List.map Term.get_id (Term.find_var_refs Term.Eigen (List.map snd alist)) in
...
let rec copy_formula f = 
  ...
  | Exists(vs,body) ->
     ...
     exists vs body'
  | Ctx(vs,body) ->
     ctx vs (copy_formula body)
  | Imp(l,r) ->
     imp (copy_formula l) (copy_formula r)
  | And(l,r) ->
     f_and (copy_formula l) (copy_formula r)
  | Or(l,r) ->
     f_or (copy_formula l) (copy_formula r)
  | _ -> f
...
let rec replace_ctx_vars ctxvar_subst f =
  ...
  | Top | Bottom | Prop _ -> f
  | Imp(l,r) ->
    imp (replace_ctx_vars ctxvar_subst l) (replace_ctx_vars ctxvar_subst r)
  | And(l,r) ->
    f_and (replace_ctx_vars ctxvar_subst l) (replace_ctx_vars ctxvar_subst r)
  | Or(l,r) ->
     f_or (replace_ctx_vars ctxvar_subst l) (replace_ctx_vars ctxvar_subst r)
  | All(vs,f) ->
     forall vs (replace_ctx_vars ctxvar_subst f)
  | Exists(vs,f) ->
     exists vs (replace_ctx_vars ctxvar_subst f)
  | Ctx(bndrs,f') ->
    ...
    let bnd_subst =
      ...
      List.map
        ...
        bndrs
    in
    ctx (List.map2 (fun (_,s) (_,g) -> (Context.get_ctx_var g,s)) bndrs bnd_subst)
  ...
  | Atm(g,m,a,ann) ->
    ...
    then
      ...
      | Some g' ->
          atm ~ann:ann (Context.replace_ctx_vars [(Context.get_ctx_var g, g')] g) m a
      | None -> f)
    else
      f
