...
let cp_sequent sq = {sq with vars = sq.vars; ctxvars = (List.map cp_cvar_entry sq.ctxvars)}
...
let add_var sequent (id,tm) =
  ...
  then
    ...
    sequent.vars <- List.map (fun (name,t) -> if id = name then (name,tm) else (name,t)) sequent.vars
...
let ctxvar_mem cvars n =
  List.exists (fun (var,_,_) -> Context.ctx_var_eq n var) cvars
...
let replace_seq_vars subst sequent =
    ...
    sequent.ctxvars <- List.map (fun (v,rstrct,ctxty) -> (v,rstrct,Context.replace_ctx_typ_vars subst ctxty)) sequent.ctxvars;
    sequent.hyps <- List.map (fun h -> {id=h.id; tag=h.tag; formula=Formula.replace_formula_vars subst h.formula}) sequent.hyps;
...
let add_hyp sequent ?name form =
  let name = fresh_hyp_name sequent begin
                 match name with
                 | None -> ""
                 | Some name -> name
               end in
  sequent.hyps <- List.append sequent.hyps
     [{ id = name ; formula = form ; tag = Explicit }]

let get_hyp sequent name =
  let hyp =
      List.find (fun h -> h.id = name) sequent.hyps
  in
  hyp
...
let rec remove_all f l =
  ...
  | h::tl ->
     if f h
     then remove_all f tl
     else h :: (remove_all f tl)
    ...
    | Formula.Atm(g,m,a,ann) ->
     (match Term.observe (Term.hnorm a) with
       | Term.Pi((v,typ)::bndrs, body) ->
          ...
          let a' = Term.replace_term_vars [(v.Term.name,Term.eta_expand name)] (Term.pi bndrs body) in
          aux (Formula.atm ~ann:ann g' m' a' )
       | _ -> formula)
    | _ -> formula
  in
  aux formula 
...
let exists_left sequent formula =
  let rec aux formula =
    ...
    | Formula.Exists(vs,f) ->
       ...
       aux (Formula.replace_formula_vars
