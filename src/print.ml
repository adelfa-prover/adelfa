...
let rec pr_ty_literal ppf = function
  | Ty(tys, bty) ->
     Format.fprintf ppf "Ty([%a],%a)" pr_tylst_literal tys pr_str bty
and pr_tylst_literal ppf = function
  | [] -> ()
  | (ty::[]) -> pr_ty_literal ppf ty
  | ty :: tys ->
     Format.fprintf ppf "%a,@ %a" pr_ty_literal ty pr_tylst_literal tys
...
let rec pr_term_literal ppf t =
  match t with
  | Var(v) ->
    Format.fprintf ppf "@[<2>Var(@,%a@,)@]" pr_var_literal v
  | DB(i) ->
    Format.fprintf ppf "@[<2>DB(@,%a@,)@]" pr_str (string_of_int i)
  | Lam(idtys, tm) ->
  ...
  | Type ->
    pr_str ppf "Type"
  | Ptr p ->
    Format.fprintf ppf "@[<2>Ptr(@,%a@,)@]" pr_ptr_literal p
  | Susp _ ->
    pr_term_literal ppf (hnorm t)
and pr_var_literal ppf v =
  let string_of_tag = function
    ...
    | Nominal -> "Nominal"
  in 
  Format.fprintf ppf "name=%a,@,tag=%a,@,ts=%a,@,ty=%a" pr_str v.name pr_str (string_of_tag v.tag) pr_str (string_of_int v.ts) pr_ty_literal v.ty
and pr_ptr_literal ppf p =
  match !p with
  | V v ->
    Format.fprintf ppf "V(@,Var(%a))" pr_var_literal v
  | T t ->
    Format.fprintf ppf "T(@,%a)" pr_term_literal t
and pr_lfidlst ppf lfidtys =
  match lfidtys with
  | [] -> Format.fprintf ppf "@,"
  | ((v,ty)::[]) ->
    Format.fprintf ppf "@,(%a,%a)@," pr_str v.Term.name pr_term_literal ty
  | ((v,ty)::lfidtys') ->
    Format.fprintf ppf "@,(%a,%a)@,,%a" pr_str v.Term.name pr_term_literal ty pr_lfidlst lfidtys'
and pr_idlst ppf idtys =
  match idtys with
  | [] -> Format.fprintf ppf "@,"
  | ((id,ty)::[]) ->
    Format.fprintf ppf "%a:%a" pr_str id pr_ty_literal ty
  | ((id,ty)::idtys') ->
...
and pr_term' cx ppf t =
  ...
  | _ -> assert false
and pr_db cx ppf i =
  pr_str ppf (db_to_string cx i)
and pr_app cx ppf h tms =
  let rec pr_args ppf tms =
    match tms with
    | [] -> ()
    | (t :: tms') ->
       Format.fprintf ppf "@ %a%a" (pr_term' cx) t pr_args tms'
  in
...
and pr_pi cx ppf idtys tm =
  let rec pr_bndr cx ppf idtys =
    ...
    | [] -> pr_term cx ppf tm
    | (v,k)::idtys' ->
       Format.fprintf ppf "@[<2>{@,%a@,:@,%a@,}%a@]" pr_var v (pr_term cx) k (pr_bndr ((v.name)::cx)) idtys'
  in
  pr_bndr cx ppf idtys
...
let rec pr_varlst ppf lst =
  ...
  | [] -> ()
  | ((id,t) :: []) ->
    Format.fprintf ppf "%a@,:@,%a" pr_str id pr_ty (Term.get_var_ty t)
  (* pr_str ppf id *)
  | ((id,t) :: vs) ->
...
let rec pr_block ppf b =
  ...
  | [] -> ()
  | ((v,tm)::[]) ->
    Format.fprintf ppf "%a@,:@,%a" pr_str v.name (pr_term []) tm
  | ((v,tm)::b') ->
...
let rec pr_blocks ppf blocks =
  ...
  | [] -> ()
  | b::[] ->
    Format.fprintf ppf "(%a)" pr_block b
  | b::blocks' ->
    Format.fprintf ppf "%a,@ (%a)" pr_block b pr_blocks blocks' 
...
let rec pr_ctxvarlst ppf (lst:Sequent.cvar_entry list) =
  ...
  | ((id,ns,ty) :: vs) ->
    Format.fprintf ppf "%a{%a}@,:@,%a,@ %a" pr_str id pr_strlst (!ns) pr_ctxty ty pr_ctxvarlst vs 
...
let pr_ctxentry ppf (v,tm) =
  Format.fprintf ppf "%a@,:@,%a" pr_var v (pr_term []) tm
                 
let rec pr_ctxexpr ppf = function
  | Context.Nil -> ()
  | Context.Var(v) -> pr_ctxvar ppf v
  | Context.Ctx(Context.Nil, entry) ->
     pr_ctxentry ppf entry
...
let rec pr_formula ppf = function
  ...
  | Formula.Atm(g,m,a,ann) ->
     Format.fprintf ppf "@[<2>{%a@,@ |-@ @,%a@ :@ %a}%a@]" pr_ctxexpr g (pr_term []) m (pr_term []) a pr_ann ann
  | Formula.Ctx(bndrs,f) ->
...
and pr_propargs ppf = function
  | [] -> ()
  | (arg::args') ->
     Format.fprintf ppf "@ %a%a" (pr_term []) arg pr_propargs args'
let pr_hyp ppf hyp =
  if hyp.Sequent.tag = Sequent.Explicit
  then
    Format.fprintf ppf "%a:@,%a" pr_str hyp.Sequent.id pr_formula hyp.Sequent.formula
  else
    ()
...
let rec pr_hyps ppf hyps =
  ...
  | [] -> ()
  | (h::hyps') ->
     Format.fprintf ppf "@[<4>%a@]@.%a" pr_hyp h pr_hyps hyps'
...
let pr_sequent ppf seq =
  ...
  (if evars = []
   then ()
      ...
      Format.fprintf ppf "Vars: @[<2>%a@]@." pr_varlst (List.filter Term.is_uninstantiated evars));
  (if nvars = []
   then ()
   else
      Format.fprintf ppf "Nominals: @[<2>%a@]@." pr_varlst (nvars));
  (if seq.Sequent.ctxvars = []
   then ()
   else
      Format.fprintf ppf "Contexts: @[<2>%a@]@." pr_ctxvarlst (seq.Sequent.ctxvars)); 
  Format.fprintf ppf "@[%a@]@.==================================@." pr_hyps (seq.Sequent.hyps);
...
let rec pr_uform ppf = function
  ...
  | Uterms.UAtm(uctx, utm, uty, ann) ->
     Format.fprintf ppf "@[<2>{@,%a@,%a@ :@ %a}%a" pr_uctx uctx pr_uterm utm pr_uterm uty pr_ann ann
  | Uterms.UProp(utm) ->
     pr_uterm ppf utm
...
let pr_aid ppf = function
  | (id, Some(ty)) ->
     Format.fprintf ppf "@[<2>%a@ :@ %a@]" pr_str id pr_ty ty
  | (id, None) -> pr_str ppf id

let pr_udef ppf (f1, f2) =
...
let rec pr_command ppf = function
  ...
  | Uterms.Ind(i) ->
     Format.fprintf ppf "induction on %a." pr_str (string_of_int i)
  | Uterms.Apply(name, args, []) ->
     Format.fprintf ppf "@[<4>apply %a %a.@]" pr_clearable name pr_args args
  | Uterms.Apply(name, args, withs) ->
     Format.fprintf ppf "@[<4>apply %a %a with %a.@]" pr_clearable name pr_args args pr_withs withs
  | Uterms.Case(name) ->
     Format.fprintf ppf "cases %a." pr_clearable name
  | Uterms.Exists(utm) ->
     Format.fprintf ppf "@[<4>exists %a.@]" pr_uterm utm
  ...
  | Uterms.Strengthen(name) ->
     Format.fprintf ppf "strengthen %a." pr_clearable name
  | Uterms.Inst(name,withs) ->
     Format.fprintf ppf "@[<4>inst %a with %a.@]" pr_clearable name pr_withs withs
  | Uterms.Prune(id) ->
     Format.fprintf ppf "prune %a." pr_str id
  | Uterms.Unfold(None, []) ->
     pr_str ppf "unfold."
  | Uterms.Unfold(Some(hid), []) ->
     Format.fprintf ppf "unfold %a." pr_str hid
  ...
  | Uterms.AppDfn(did,None,[]) ->
     Format.fprintf ppf "appdfn %a." pr_str did
  ...
  | Uterms.AppDfn(did,Some(hid),withs) ->
     Format.fprintf ppf "@[<4>appdfn %a to %a with %a.@]" pr_str did pr_str hid pr_withs withs
and pr_clearable ppf = function
  | Keep(id)
  | Remove(id) -> pr_str ppf id
and pr_clearablelist ppf = function
  | [] -> ()
  | (clearable::[]) ->
     Format.fprintf ppf "%a" pr_clearable clearable
  | (clearable::clearables) ->
     Format.fprintf ppf "%a@ %a" pr_clearable clearable pr_clearablelist clearables
and pr_args ppf = function
  | [] -> ()
  | args ->
    Format.fprintf ppf "to@ %a" pr_clearablelist args
...
let rec pr_uschema ppf =
  ...
  | [] -> ()
  | (ublockschema:: []) ->
     Format.fprintf ppf "%a" pr_ublockschema ublockschema
