...
val trans_term : Signature.signature ->
                 (Term.id * (Term.term option ref)) list ->
                 (Term.id * (Term.term option ref)) list -> 
                 (Term.id * (Term.term option ref)) list -> (Term.id * (Term.term option ref)) list  ->
                 Type.ty option -> Uterms.uterm -> (Term.term * ((Term.id * (Term.term option ref)) list))
val trans_formula : Signature.signature ->
                ...
                (Term.id * (Term.term option ref)) list ->
                Uterms.uformula -> Formula.formula
val trans_ctx : Signature.signature ->
                (Term.id * (Term.term option ref)) list ->
                (Term.id * (Term.term option ref)) list ->
                Sequent.cvar_entry list ->
                (Term.id * (Term.term option ref)) list -> Uterms.uctx ->
                (Context.ctx_expr * ((Term.id * (Term.term option ref)) list))
val trans_schema : Signature.signature ->
                   Uterms.uschema -> Context.ctx_schema
val trans_dfn : Signature.signature ->  (string * Context.ctx_schema) list ->
                (string * Type.ty) list -> Term.id -> Type.ty -> Uterms.udef list ->
                  Definition.dfn
