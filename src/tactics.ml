...
type case = {vars_case : (Term.id * Term.term) list;
...
let fresh_nameless_alist ~support ~tag ~ts tids =
  ...
  List.map
    (fun (n,ty) ->
       (n, app (eta_expand (fresh ~tag ~ts (Type.tyarrow ntys ty))) support))
    tids
...
let freshen_block ~used ?(support=[]) (Context.B(schema_vars,entries)) =
  let (alist, vars) = Formula.fresh_raised_alist ~tag:Eigen ~used ~ts:1 ~support schema_vars in
...
let context_instance schemas nvars ctxvar_ctx bound_ctxvars g1 g2 =
  ...
  let rec aux g1 g2 =
    ...
    | Context.Var v, g2 ->
      ...
      then
        let Context.CtxTy(s,[]) = Option.get ctxty in
    ...
    | Context.Ctx(g1',(n1,ty1)), Context.Ctx(g2',(n2,ty2)) ->
       if n1 = n2
       then
         (if Unify.try_right_unify ~used:nvars ty1 ty2
          then
            aux g1' g2'
          else
            None)
  ...
  let rec inst f1 f2 =
    ...
    | Formula.Ctx(bndrs1,f1'), Formula.Ctx(bndrs2,f2') ->
      ...
      then
        ...
        if List.fold_left2 (fun b (_,s1) (_,s2) -> b &&  (s1 = s2)) true bndrs1 bndrs2
        then
          let subst =
            List.map2 (fun (id1,_) (id2,_) -> (id1,Context.Var id2))
                      bndrs1
                      bndrs2
          in
    ...
    | Formula.Exists(vs1,f1'), Formula.Exists(vs2,f2') ->
      ...
      then
        ...
        if List.fold_left2 (fun b idty1 idty2 -> b && idty1 = idty2) true vs1 vs2
        then
          let f = Formula.replace_formula_vars
    ...
    | Formula.Atm(g1,m1,a1,_), Formula.Atm(g2,m2,a2,_) ->
          (if Unify.try_right_unify ~used:nvars m1 m2 &&
           ...
           then
             context_instance schemas nvars ctxvar_ctx bound_ctxvars g1 g2
           else
             None)
            ...
            (Term.unwind_state
               (fun perm_support_t1 ->
                 ...
                 try
                   ...
                   in 
                   if Option.is_some subst
                   then
                     sc (subst,Term.get_bind_state ())
                 with
...
let all_meta_right_permute_unify_ctxexpr ~sc schemas nvars ctxvar_ctx new_ctxvar_ctx g1 g2 =
  ...
  let support_g2 = Formula.context_support_sans (new_ctxvar_ctx @ ctxvar_ctx) g2 in
    if List.length support_g1 < List.length support_g2 then
      (* Ground term cannot have more nominals than logic term *)
      ()
    else
...
let instance_of nvars ctxvar_ctx f1 f2 =
  ...
  all_meta_right_permute_unify ~sc:(fun s -> raise Success) [] nvars ctxvar_ctx [] f1' f2
...
let decompose_kinding lf_sig used ctx typ =
  ...
  let rec decompose ctx typ =
    ...
    | Pi(tyctx,ty) ->
       ...
       let (nvar, used') = fresh_wrt 3 Nominal "n" (erase typ) !used in
       used := used';
       List.append
         (decompose ctx typ)
         (decompose
    ...
    | App(h, args) when (is_var Constant (observe (hnorm h))) ->
       ...
       then
         ...
         let Pi(tyargs,Type) = ty_decl.Signature.kind in
         ...
         List.mapi
           (fun i ((v,typ),arg) ->
            let typ' =
              replace_term_vars
                (List.map
                   (fun ((v,t),a) -> (v.Term.name, a))
                   (List.take i tyargs_args))
                typ
            in
            Formula.atm ctx arg typ')
           tyargs_args
       else
         raise (InvalidName(get_id (observe (hnorm h))))
...
let satisfies r1 r2 =
  match r1, r2 with
  | Formula.LT i, Formula.LT j
  | Formula.LT i, Formula.EQ j
  | Formula.EQ i, Formula.EQ j when i = j -> true
...
let search signature sequent =
  ...
  let check_context_names g =
    let explicit_names = (List.map (fun (x,y) -> x.Term.name) (Context.get_explicit g)) in
     ...
     then
       List.for_all
         (fun x ->
            List.mem
              x
              (Sequent.get_ctxvar_restricted (Sequent.ctxvar_lookup sequent.ctxvars (Context.get_ctx_var g))))
         explicit_names
     else
       true)
  in
  ...
  let rec search_aux (subgoals:(unit -> unit) list) =
    ...
    let rec check_context used g =
      let hyp_ctxexprs =
        List.map
          (fun x ->
            let Formula.Atm(g,_,_,_) = x.formula in
            g)
          (List.filter
             (fun x ->
      ...
      in
      let support_g = Formula.context_support_sans (Sequent.get_cvar_tys sequent.ctxvars) g in
      ...
      List.iter
        (fun hyp_g ->
          ...
          then
            let support_hypg = Formula.context_support_sans (Sequent.get_cvar_tys sequent.ctxvars) hyp_g in
      ...
      | Context.Var _ -> ()
      | Context.Ctx(g',(v,typ)) ->
        let save_seq, bind_state =
          Sequent.cp_sequent sequent,
          Term.get_bind_state ()
        in
    ...
    let try_match () =
      let support_goal = Formula.formula_support_sans (Sequent.get_cvar_tys sequent.ctxvars) sequent.goal in
      List.iter
        (fun h ->
          let f = h.formula in
          ...
          | Formula.Atm(g,m,a,ann) ->
            ...
            then 
              let support_hyp =
                Formula.formula_support_sans (Sequent.get_cvar_tys sequent.ctxvars) f in
    ...
    let lf_step () =
      ...
      let make_subgoals g ann args bndrs body =
        let rec mk_sg args bndrs =
          ...
          | (arg::[]), bndrs when List.length bndrs > 1 ->
             (* arg : pi(bndrs,body) *)
             let subgoal =
               let seq = Sequent.cp_sequent sequent in
               (fun () ->
                sequent.goal <- Formula.Atm(g,arg, pi bndrs body,ann))
             in
             [subgoal]
          | (arg::args'), ((v,ty)::bndrs') ->
             let subgoal =
               let seq = Sequent.cp_sequent sequent in
               (fun () ->
                sequent.goal <- Formula.Atm(g,arg,ty,ann))
             in
             subgoal::(mk_sg args' (List.map (fun (b,ty) -> (b,Term.replace_term_vars [(v.name,arg)] ty)) bndrs'))
        in
        mk_sg args bndrs
      in
      ...
      let Formula.Atm(g,m,a,ann) = sequent.goal in
      match Term.observe (Term.hnorm m) with
      | Term.Var _ as h
      | Term.App(h, []) ->
        (* leaf analysis in this case *)
          let new_tms, new_lftys, hd_ty =
          ...
          (try
              ...
              then
                check_context
                  (Formula.get_formula_used_nominals (Sequent.get_cvar_tys sequent.ctxvars) sequent.goal)
                  g
              else
                ()
           with
            ...
            match Term.observe (Term.hnorm h) with
             ...
             | Term.Var v when v.tag=Term.Nominal ->
                ...
                (* the names bound by Pis are all fresh. *)
                  ...
                  |> freshen_type ~used:sequent.vars ~support:[]
          in
          let hd_ty = pi (List.map2 (fun t ty -> ((Term.term_to_var t),ty)) new_tms new_lftys) target_ty in
          (try
       ...
       let rec aux f =
         (match f with
          | Formula.Atm(g,m,a,ann) ->
              ...
              else  raise (InvalidFormula(sequent.goal, "Cannot induct on annotated formulas."))
          | _ ->
            raise (InvalidFormula(sequent.goal,
                                  "Formula must be an atomic assumption for induction")))
       in
       aux l
    | Formula.Imp(l,r) ->
...
(* Possible raising optimization:
     in Psij' we might check which variables in Psi have a (strict?)
     occurence in G1,...,Gj and avoid raising these variables because 
     such dependencies would be ill-formed. *)
let addBlock (seq:Sequent.sequent) (tycvar:Sequent.cvar_entry) (bl_schm :Context.block_schema) (names:Term.term list) (usable:Term.term list) (j:int) (i:int) : (Sequent.sequent * Term.term * Term.term) =
  ...
  let new_names = List.minus ~cmp:Term.eq names (List.map snd (Sequent.get_nominals seq)) in
  let psij' =
    if new_names = []
    then
      psi
  ...
  in
  let thetaj' =
    List.map2 (fun (id,_) (_,tm) -> (id, (Term.app tm new_names))) psi psij'
  in
  (* generating the new block and related eigenvariable context *)
  let Context.B(schema_vars,entries) = bl_schm in
  let Context.CtxTy(schema,blocks) = Sequent.get_ctxvar_ty tycvar in
  ...
  let g =
    List.map2
      (fun n lfty ->
       (Term.term_to_var n,
        (replace_term_vars thetaj''
                           (replace_term_vars nominal_inst
                                              lfty))))
      names
      sch_typs
  in
  ...
  let _ =
    List.iter (Sequent.add_var seq)
              (List.map Term.term_to_pair new_names)
  in
  ...
  let (gamma, ngamma, (Context.CtxTy(schema,blocks))) =
 ...
 *)
let rec namesLists (atys:Type.ty list) (names:Term.term list) (prohibited:Term.term list) : (Term.term list) list =
  match atys with
  | [] -> [[]]
  | (a::atys') ->
     let new_name =
     ...
     in
     let avail_choices =
       List.filter (fun v -> Term.get_var_ty v = a) (List.minus ~cmp:Term.eq names prohibited)
     in
     let pick n =
       let lsts_new = namesLists atys' names (n :: prohibited) in
       List.map (fun l -> n::l) lsts_new
     in
 ...
 *)
let allBlocks (seq:Sequent.sequent) (g:Context.ctx_expr) (tycvar:Sequent.cvar_entry) (bl_schm:Context.block_schema) : (Sequent.sequent * Term.term * Term.term) list =
  let Context.B(atyids, entries) = bl_schm in
  let entry_atys = List.map (fun (v,_) -> v.ty) entries in
  let noms = Sequent.get_nominals seq in
  let n_gamma =
    List.map
      (fun n -> List.assoc n noms)
      (Sequent.get_ctxvar_restricted tycvar)
  in
  let prohibited =
    Context.ctxexpr_to_ctx (Sequent.get_cvar_tys seq.ctxvars) g
    |> List.map (fun (v,ty) -> Term.var_to_term v)
  in
  let usable =
    List.minus ~cmp:Term.eq
               (List.minus ~cmp:Term.eq
                           (List.map snd noms)
                           n_gamma)
               prohibited
    ...
    (fun j ->
         List.map
           (fun names ->
                List.map
                  (fun i ->
                   addBlock seq tycvar bl_schm names (List.minus usable names) j i)
                  is)
           namelsts
         |> List.flatten)
    js
...
*)   
let implicitHeads seq schemas (g:Context.ctx_expr):(Sequent.sequent * Term.term * Term.term) list =
...
let sigHeads lf_sig seq :(Sequent.sequent * Term.term * Term.term) list =
  ...
  List.map
    (fun obj ->
     (seq,
       (Term.const obj.Signature.name (Term.erase obj.Signature.typ)),
       obj.Signature.typ))
    objs
...
let heads lf_sig schemas seq g =
  ...
  if Context.has_var g
  then
    sig_heads @ ex_heads @ (implicitHeads seq schemas g)
  else
    sig_heads @ ex_heads
      ...
      List.map2
       (fun t ty ->
         (Formula.Atm(g,
                      t,
                      ty,
                      Formula.reduce_inductive_annotation ann)))
        new_tms
        new_lftys 
    in
...
let cases lf_sig schemas seq id : case list =
  let hyp =
    try get_hyp seq id
    with Not_found -> raise (InvalidName id)
  in
  let (g,m,a,ann) =
  ...
  let ty_head =
    ...
    | Var v when v.tag = Constant ->
      Signature.lookup_type_decl lf_sig v.name;
      v.name
    | App(h,args) ->
       ...
       | Var(v) when v.tag = Constant ->
          Signature.lookup_type_decl lf_sig v.name;
          v.name
        | _ -> raise (InvalidFormula (hyp.formula,
                     "Atomic formula must have a base type typing judgment.")))
    | _ -> raise (InvalidFormula (hyp.formula,
                 "Atomic formula must have a base type typing judgment."))
  in
  let case_heads =
    heads lf_sig schemas seq g
      |> List.filter (fun (s,n,typ) -> (Term.get_ty_head typ) = ty_head)
  in
...
let exists sequent t =
  ...
  | Formula.Exists(((n,ty)::vs), body) ->
    if Type.eq got_ty ty
    then 
      sequent.goal <- Formula.replace_formula_vars_rename ~used:sequent.vars [(n, t)] (Formula.Exists(vs,body))
    else
      raise (InvalidTerm(t))
  | _ -> assert false
         ...
         | Formula.Imp(left, right), arg ->
            ...
            let rec norm f =
              ...
              | Formula.Exists(exists, body) ->
                 norm (freshen_nameless_bindings ~support:(List.map snd nvars) ~ts:1 exists body)
           ...
           (match !res with
            ...
            | None,_ ->
              raise (Unify.UnifyFailure Unify.Generic))
         | _ ->
...
let apply schemas sequent formula args =
  let process_bindings ctxs foralls body =
    let (new_ctxvars,body') = freshen_ctx_bindings (List.map Sequent.get_ctxvar_id sequent.ctxvars) ctxs body in
  ...
  let rec collect_bindings ctxbndrs varbndrs formula =
    ...
    | Formula.Ctx(bndrs, body) ->
       collect_bindings (ctxbndrs @ bndrs) varbndrs body
    | Formula.All(vs, body) ->
       collect_bindings ctxbndrs (varbndrs @ vs) body
    | Formula.Imp(_,_) ->
       (ctxbndrs, varbndrs, formula)
    | _ ->
...
let take_from_binders binders withs =
  ...
  in
  let binders' = List.remove_all (fun (x, ty) -> List.mem_assoc x withs) binders
  in
  (binders', withs')

let take_from_ctxbinders ctxbinders withs =
  let withs' =
    List.find_all (fun (x,t) -> List.mem_assoc x ctxbinders) withs
  in
  let binders' = List.remove_all (fun (x,ctxty) -> List.mem_assoc x withs) ctxbinders
  in
  (binders', withs')
...
let rec instantiate_withs term (vwiths, cwiths) =
  ...
  | Formula.All(idtys,f) ->
    ...
    let body = instantiate_withs (Formula.replace_formula_vars withs' f) (vwiths,cwiths) in
...
let weaken lf_sig sequent form t =
  ...
  | Formula.Atm(g,m,a,ann) ->
     ...
     let (save_seq,bind_state) = Sequent.cp_sequent sequent,
                                 Term.get_bind_state () in
...
let permute_ctx (Formula.Atm(g,m,a,ann) as f) g' =
  (* need to verify that 
  ...
  let rec check_dependencies = function
    ...
    | ((v::vs),(typ::typs)) ->
      ((if List.mem (v.Term.name) (List.map fst (Term.get_used typs))
        then
           raise (InvalidCtxPermutation "Later contet items cannot depend on later name")) ;
  ...
  (if not (List.length g_used_ctxvars = List.length g'_used_ctxvars)
   then
     raise (InvalidCtxPermutation "context expression must contain the same number of context variables."));
  List.iter
    (fun cvar ->
  ...
  List.iter
    (fun g'_entry ->
       ...
       then
         (let (v,t) = g'_entry in
          raise (InvalidCtxPermutation ("not found: "^v.Term.name^". all entried must be in both contexts")))
       else ())
    g'_explicit ;
  check_dependencies (List.split g'_explicit);
  Formula.atm ~ann:ann g' m a
                                
let strengthen ctxvars (Formula.Atm((Context.Ctx(g,(v,t))),m,a,ann)) =
  let used = Formula.formula_support (Sequent.get_cvar_tys ctxvars) (Formula.Atm(g,m,a,ann)) in
  if List.exists (fun t -> Term.term_to_name t = v.Term.name) used
  then
    None
  else
    Some(Formula.Atm(g,m,a,ann))
...
let inst lf_sig sequent (Formula.Atm(g,m,a,ann) as f) ([(n, t)] as subst) =
(* split the context g based on location on n into g1,n:b,g2.
  ...
  |Success ->
    (
      let _ = Sequent.assign_sequent sequent save_seq;
              Term.set_bind_state bind_state in
      let g2' = List.map
                  (fun (id,ty) ->
                   (id, Term.replace_term_vars ~tag:Term.Nominal subst ty))
                  g2
      in
...
(* Prune will identify dependencies which are impossible given the restriction
   ...
   context variable. *)
let prune sequent (Formula.Atm(g,m,a,ann)) =
  if Context.has_var g
  then
    let Term.App(hd,args) = Term.norm m in
    let gamma = Context.get_ctx_var g in
    let cvar = Sequent.ctxvar_lookup sequent.ctxvars gamma in
    let bound_names =
      List.map (fun (v,_) -> v.Term.name) (Context.ctxexpr_to_ctx (Sequent.get_cvar_tys sequent.ctxvars) g)
    in
    let restricted =
      List.minus
        (Sequent.get_ctxvar_restricted cvar)
        bound_names
    in
    (if restricted = []
     then
       ...
       let ns =
         List.map Term.eta_normalize args 
       in
       let to_prune =
         List.map (fun t -> List.mem (Term.get_id t) restricted) args 
       in
       let args' =
         (List.combine to_prune ns)
         |> List.filter (fun (b,_) -> not b)
         |> List.map (fun (_,v) -> Term.nominal_var (Term.get_id v) (Term.get_var_ty v))
       in
       let atys' =
         (List.combine to_prune atys)
         |> List.filter (fun (b,_) -> not b)
         |> List.map snd
       in
       let new_ty = Type.Ty(atys',id) in
       let new_term =
         Term.var Eigen (Term.get_id hd) 1 new_ty
       in
       let subst =
         [(Term.get_id hd),
          (List.fold_left (fun body n ->
                               abstract (Term.get_id n) (Term.get_var_ty n) body)
