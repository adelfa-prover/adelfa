...
val fresh_alist : used:(id * term) list -> tag:Term.tag -> ts:int -> (Term.id * Type.ty) list -> (Term.id * term) list
val raise_type : term list -> Type.ty -> Type.ty
val fresh_raised_alist : used:(id * term) list -> tag:Term.tag -> ts:int -> support:term list -> (Term.id * Type.ty) list ->
...
val replace_formula_vars_rename : used:(id * term) list -> (id * term) list -> formula -> formula
...
val context_support : (Context.ctx_var * Context.ctx_typ) list -> Context.ctx_expr -> term list
val context_support_sans : (Context.ctx_var * Context.ctx_typ) list -> Context.ctx_expr -> term list
  
val formula_support : (Context.ctx_var * Context.ctx_typ) list ->
  formula -> term list
val get_formula_used : (Context.ctx_var * Context.ctx_typ) list -> formula  -> (id * term) list
val get_formula_used_nominals : (Context.ctx_var * Context.ctx_typ) list -> formula -> (id * term) list
val formula_support_sans : (Context.ctx_var * Context.ctx_typ) list ->
  formula -> term list
